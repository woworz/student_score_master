package com.example.score.service.Score.impl;

import com.github.pagehelper.PageRowBounds;
import com.example.score.dao.Score.ScoreMapper;
import com.example.score.dto.Course;
import com.example.score.dto.Score;
import com.example.score.service.Score.ScoreService;
...
public class ScoreServiceImpl implements ScoreService {
  ...
  public void addEntry(List<Score> list) {
    for (Score score : list) {
      try {
        // 检查scoreByUser是否为空或非法值
        if (score.getScoreByUser() == null || score.getScoreByUser().trim().isEmpty()) {
          continue; // 跳过空值
        }
        ...
        if (scoreByUser < 0) {
          scoreByUser = 0;
        } else if (scoreByUser > score.getScore()) {
          scoreByUser = score.getScore();
        }
        ...
        score.setScoreByUser(String.valueOf((int)scoreByUser)); // 确保存储的是整数
        ...
        // 处理数字格式异常
        System.out.println("分数格式错误: " + score.getScoreByUser() + " 对于学生: " + score.getNo());
        // 可以选择跳过这条记录或设置默认值
        continue;
      } catch (Exception e) {
        // 处理其他异常
        System.out.println("处理成绩时发生错误: " + e.getMessage());
        e.printStackTrace();
        continue;
      }
    }
