...
import com.example.score.utils.AuthenticationInterceptor;
...
public class WebConfig extends WebMvcConfigurationSupport {
  ...
  public void addInterceptors(InterceptorRegistry registry) {
    ...
    //添加自定义拦截器和拦截路径，此处对所有请求进行拦截，除了登录界面和登录接口以及注册相关接口
    registry.addInterceptor(appInterceptor())
        .addPathPatterns("/api/sms/**")//添加拦截路径,拦截所有
        .excludePathPatterns("/login") // 排除的拦截路径
        .excludePathPatterns("/api/sms/user/register") // 排除注册接口
        .excludePathPatterns("/api/sms/user/register/check/**"); // 排除用户名检查接口
